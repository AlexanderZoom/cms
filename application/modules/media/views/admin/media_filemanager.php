...
var loadConfigFile = function (type) {
	...
	if(type == 'user') {
		if($.urlParam('config') != 0) {
			var url = '<?php echo $cfgUrl; ?>' + $.urlParam('config');
			userconfig = $.urlParam('config');
		} else {
			var url = '<?php echo $cfgUrl; ?>';
    ...
    $.ajax({
        ...
        'success': function (data) {
        	if (data['result'] != 'OK') handleError ('Error #' + data['error_code'] +': '+ data['error_description']);
            json = data['data'];
        }
    });
    return json;
...
if($.urlParam('langCode') != 0) {
    if(file_exists ('/admin-content/js/filemanager/languages/'  + $.urlParam('langCode') + '.js')) {
        config.options.culture = $.urlParam('langCode');
    } else {
        var urlLang = $.urlParam('langCode').substring(0, 2);
        if(file_exists ('/admin-content/js/filemanager/languages/'  + urlLang + '.js')) config.options.culture = urlLang;
    }
}

var lg = [];
$.ajax({
  url: '/admin-content/js/filemanager/languages/'  + config.options.culture + '.js',
  async: false,
  dataType: 'json',
...
var getVideoPlayer = function(data) {
	var code  = '<video width=' + config.videos.videosPlayerWidth + ' height=' + config.videos.videosPlayerHeight + ' src="' + data['RealPath'] + '" controls="controls">';
...
var getAudioPlayer = function(data) {
	var code  = '<audio src="' + data['RealPath'] + '" controls="controls">';
		code += '<img src="' + data['Preview'] + '" />';
		code += '</audio>';
...
var getPdfReader = function(data) {
	var code  = '<iframe id="fm-pdf-viewer" src = "/admin-content/js/filemanager/ViewerJS/index.html#' + data['RealPath'] + '" width="' + config.pdfs.pdfsReaderWidth + '" height="' + config.pdfs.pdfsReaderHeight + '" allowfullscreen webkitallowfullscreen></iframe>';
			...
			if(fname != ''){
				...
				$.getJSON(fileConnector + '?mode=addfolder&path=' + $('#currentpath').val() + '&config=' + userconfig + '&name=' + encodeURIComponent(foldername) + '&time=' + d.getMilliseconds(), function(result){
					if(result['result'] == 'OK'){
						...
						$.prompt(result['error_description']);
					}				
				});
			} else {
...
var getFileInfo = function(file) {
	...
	$.getJSON(fileConnector + '?mode=getinfo&path=' + encodeURIComponent(file)  + '&config=' + userconfig + '&time=' + d.getMilliseconds(), function(data){
		if(data['result'] == 'OK'){
			...
			if(config.options.baseUrl !== false ) {
				var url = smartPath(baseUrl, data['RealPath'].replace(fileRoot,""));
			} else {
			...
			if(data['Protected']==0) {
				...
				loadJS('/admin-content/js/filemanager/zeroclipboard/copy.js?d' + d.getMilliseconds());
...
var getFolderInfo = function(path) {
	...
	var loading = '<img id="activity" src="/admin-content/js/filemanager/themes/' + config.options.theme + '/images/wait30trans.gif" width="30" height="30" />';
	...
	$.getJSON(url, function(data){
		...
		if (data['result'] != 'OK'){
			 handleError ('Error #' + data['error_code'] +': '+ data['error_description']);
		...
		if(data){
			...
			if($('#fileinfo').data('view') == 'grid'){
				...
				for(key in data){
					...
					if(props['Size'] && props['Size'] != '') totalSize += props['Size'] * 1;
...
$(function(){
	...
	// Loading theme
	loadCSS('/admin-content/js/filemanager/themes/' + config.options.theme + '/styles/filemanager.css');
	$.ajax({
	    url:'/admin-content/js/filemanager/themes/' + config.options.theme + '/styles/ie.css',
	    async: false,
	    success: function(data)
	    {
	        $('head').append(data);
	...
	loadJS('/admin-content/js/filemanager/zeroclipboard/dist/ZeroClipboard.js');
	
	// Loading CodeMirror if enabled for online edition
	if(config.edit.enabled) {
		loadCSS('/admin-content/js/filemanager/CodeMirror/lib/codemirror.css');
		loadCSS('/admin-content/js/filemanager/CodeMirror/theme/' + config.edit.theme + '.css');
		loadJS('/admin-content/js/filemanager/CodeMirror/lib/codemirror.js');
		loadJS('/admin-content/js/filemanager/CodeMirror/addon/selection/active-line.js');
		loadCSS('/admin-content/js/filemanager/CodeMirror/addon/display/fullscreen.css');
		loadJS('/admin-content/js/filemanager/CodeMirror/addon/display/fullscreen.js');
		loadJS('/admin-content/js/filemanager/CodeMirror/dynamic-mode.js');
	}
	...
	/** load searchbox */
	if(config.options.searchBox === true)  {
		loadJS("/admin-content/js/filemanager/filemanager.liveSearch.js");
	} else {
		$('#search').remove();
	}
	...
	if(config.upload.multiple) {
		...
		loadCSS('/admin-content/js/filemanager/dropzone/downloads/css/dropzone.css');
		loadJS('/admin-content/js/filemanager/dropzone/downloads/dropzone.js');
	...
	if(config.customScrollbar.enabled) {
		loadCSS('/admin-content/js/filemanager/custom-scrollbar-plugin/jquery.mCustomScrollbar.min.css');
		loadJS('/admin-content/js/filemanager/custom-scrollbar-plugin/jquery.mCustomScrollbar.concat.min.js');
