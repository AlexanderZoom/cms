...
class Site_Auth_User_Db extends Site_Auth_User{
    ...
    public function logged_in($role = NULL)
    {
        ...
        if ($user instanceof Model_Site_User AND $user->loaded())
        {
    ...
    {
        if (!($user instanceof Model_Site_User)){
            $user = ORM::factory('Site_User')->where('login', '=', $user)->find();
        }
        ...
        if ($user->loaded()){
            ...
            if ($remember === true)
            {
                ...
                $token = ORM::factory('Site_User_Token')
                ->values($data)
                ->create();
    ...
    public function auto_login()
    {
        if ($token = Cookie::get($this->_config['cookie_name_autologin']))
        {
            ...
            $token = ORM::factory('Site_User_Token', array('token' => $token));
    ...
    protected function _loadUser($pk){
        $user = ORM::factory('Site_User', array('id' => $pk));
        if ($user->loaded()) return $user;
        
        return null;
    }
    
    protected function _loadAnonUser(){
    	$user = ORM::factory('Site_User', array('login' => 'anonymous'));
    	if ($user->loaded()) return $user;
    ...
    public function logout($destroy = FALSE, $logout_all = FALSE)
    {
        ...
        if ($token = Cookie::get($this->_config['cookie_name_autologin']))
        {
            ...
            if ($token->loaded() AND $logout_all)
            {
                ...
                $tokens = ORM::factory('Site_User_Token')->where('user_id','=',$token->user_id)->find_all();
