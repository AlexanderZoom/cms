...
class Controller_Admin_Groups extends Controller_Admin {
    ...
    public function action_index(){
        ...
        $this->template->content = View::factory('admin/groups');
        ...
        $optUrl = URL::site(Route::url('admin', array('controller' => 'groups', 'lang'=> $this->getLanguage())));
        ...
        $searchUrl = URL::site(Route::url('admin', array('controller' => 'groups', 'lang'=> $this->getLanguage(), 'action'=> 'index' )));
        ...
        if (isset($_REQUEST['save'])){
            ...
            if ($postValidator->check()){
                ...
                try {
                    ...
                    $this->_addAudit(Admin_Audit::TYPE_ADD, "Group {$group->code} {$group->is_disabled}");
                }
                ...
                if (!count($errors)){
                    ...
                    $this->redirect(
                        URL::site(Route::url('admin', array('controller'=>'groups', 'lang' => $this->getLanguage(), 'action'=>'index'))));
                }
            }
    ...
    public function action_edit(){
        ...
        $redirectUrl = URL::site(Route::url('admin', array('controller'=>'groups', 'lang' => $this->getLanguage(), 'action'=>'index')));
        ...
        elseif ($group->loaded() && (strtolower($group->code) == 'admin')){
            $this->setFlashError('Group Admin can not edite');
            $this->redirect($redirectUrl);
        }
        else ;
        ...
        if (isset($_REQUEST['save'])){
            ...
            if ($postValidator->check()){
                ...
                try {
                    ...
                    $this->_addAudit(Admin_Audit::TYPE_EDIT, "Group {$group->code} {$group->is_disabled}");
                }
                catch (Database_Exception $e){
                    if ($e->getCode() == 23000) $errors['login'] = ___('Group exists');
                    else throw $e;
    ...
    public function action_delete(){
        ...
        if (!empty($_REQUEST['checked']) && is_array($_REQUEST['checked']) && count($_REQUEST['checked'])){
            ...
            $groups = ORM::factory('Admin_Group')->where('id', 'in', $ids)->find_all();
            
            $admDelGroup = false;
            
            foreach ($groups as $group){
                if (strtolower($group->code) == 'admin'){
                    $this->setFlashError('Group Admin can not delete');
                    $admDelGroup = true;
                    continue;
                }
                ...
                if ($this->getRights(Admin_Access::ACCESS_DELETE, 'delete', $group->id)->result){
	                ...
	                $query = DB::delete('admin_group_user')->where('admin_group_id', '=', $group->id);
	                $query->execute();
	                
	                $query = DB::delete('admin_group_access')->where('group_id', '=', $group->id);
	                $query->execute();
    ...
    protected function _form($options = array()){
        ...
        $this->template->content = View::factory('admin/groups_form');
    ...
    protected function _breadcrumbs(){
    	...
    	if ($action != 'index'){
    		$url = URL::site(Route::url('admin', array('controller'=>'groups', 'lang' => $this->getLanguage(), 'action'=> $this->_getAction())));
