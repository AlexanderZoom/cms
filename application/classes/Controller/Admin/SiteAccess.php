...
class Controller_Admin_SiteAccess extends Controller_Admin {
    ...
    public function action_index(){
        ...
        if (!$id){
            ...
            $this->redirect(URL::site(Route::url('admin', array('controller'=>'siteGroups', 'lang' => $this->getLanguage(), 'action'=>'index'))));
        }
        ...
        /*elseif ($group->loaded() && (in_array(strtolower($group->code), array('everyone', 'banned', 'premoderate', 'user')))){
            $this->setFlashError('Group ' . $group->code .' can not edit');
            $this->redirect(URL::site(Route::url('admin', array('controller'=>'siteGroups', 'lang' => $this->getLanguage(), 'action'=>'index'))));
        }*/
        else ;
        ...
        $delim = '%';
        $emptyWord = 'EMPTY';
        $listAccessData = Site_Access::getListAccess();
        ...
        if (isset($_POST['save'])){       	
            ...
            Site_Access::deleteGroupAccessRight($id, '', '', 'EMPTY');
            
            if (count($saveRight)){            	
            	Site_Access::setGroupAccessRight($id, $saveRight);
        ...
        $this->template->title[] = ___('Access for site group :code', array(':code' => $group->code));
        $this->template->content = View::factory('admin/siteaccess_form');
        $this->template->content->listAccess = Modules::getSiteAccessClasses();
        ...
        $rights = array();
        $rowRights = Site_Access::getGroupAccess($id);
    ...
    public function action_instance(){
    	...
    	$this->template->content = View::factory('admin/siteaccess_form_instance');
    	$this->template->content->pagination = '';
    	
    	$selectedIds = array();
    	if ($isSelected){
    		$rows = Site_Access::getGroupAccessInstance($id, $package, $structure);
    		foreach ($rows as $row){
    	...
    	$options = array(
    			'url' => URL::site(Route::url('admin', array('controller' => 'siteAccess', 'lang'=> $this->getLanguage(), 'action'=> 'instance', ))),
    			'total' => $accessTotal,
    			'page'  => $page,
    			'pages' => 5,
    	...
    	if (count($accessListIds)){
    		$rows = Site_Access::getGroupAccessInstance($id, $package, $structure, $accessListIds);
    		foreach ($rows as $row){
    	...
    	if (isset($_POST['save'])){
    		...
    		Site_Access::deleteGroupAccessRight($id, $package, $structure, $accessListIds);
    		
    		if (count($saveData)){		
    			Site_Access::setGroupAccessRight($id, $saveData);
    			$this->_addAudit(Admin_Audit::TYPE_EDIT, "Access group id {$id} rights " . print_r($saveData, true));
    			$this->setFlashNotice('Access saved');
    		}
    		
    		$redirectUrl = URL::site(Route::url('admin', array('controller' => 'siteAccess', 'lang'=> $this->getLanguage(), 'action'=> 'instance' )));
    	...
    	$this->template->title[] = ___('Instance access for site group <i>:code</i> package <i>:package</i> structure <i>:structure</i>', array(':code' => $group->code, ':package' => $package, ':structure' => $structure));
    	...
    	$backUrl = URL::site(Route::url('admin', array('controller' => 'siteAccess', 'lang'=> $this->getLanguage(), 'action'=> 'index' )));
    	$backUrl = Util_Url::addParameter($backUrl, array('checked[]'=> $id));
    	
    	$resetUrl = URL::site(Route::url('admin', array('controller' => 'siteAccess', 'lang'=> $this->getLanguage(), 'action'=> 'instance' )));
    	$resetUrl = Util_Url::addParameter($resetUrl, array('gid' => $id, 'package' => $package, 'structure' => $structure));
    	
    	$selectedUrl = URL::site(Route::url('admin', array('controller' => 'siteAccess', 'lang'=> $this->getLanguage(), 'action'=> 'instance' )));
    	...
    	$searchUrl = URL::site(Route::url('admin', array('controller' => 'siteAccess', 'lang'=> $this->getLanguage(), 'action'=> 'instance' )));
    ...
    protected function _breadcrumbs(){
    	...
    	$url = URL::site(Route::url('admin', array('controller' => 'siteAccess', 'lang'=> $this->getLanguage(), 'action'=> 'index' )));
